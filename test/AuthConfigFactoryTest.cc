...
void AuthConfigFactoryTest::testCreateAuthConfig_http()
{
  ...
  netrc->addAuthenticator(std::make_unique<Authenticator>(
      "localhost", "localhostuser", "localhostpass", "localhostacct"));
  netrc->addAuthenticator(std::make_unique<DefaultAuthenticator>(
...
void AuthConfigFactoryTest::testCreateAuthConfig_ftp()
{
  ...
  netrc->addAuthenticator(std::make_unique<Authenticator>("localhost", "aria2user",
                                                     "netrcpass", "netrcacct"));
  factory.setNetrc(std::move(netrc));
...
{
  return std::make_unique<BasicCred>(user, password, host, port, path, activated);
}
} // namespace

...
void addTwoDownloadContext(BtRegistry& btRegistry)
{
  ...
  auto btObject1 = std::make_unique<BtObject>();
  btObject1->downloadContext = dctx1;
  auto btObject2 = std::make_unique<BtObject>();
...
void BtRegistryTest::testGetDownloadContext_infoHash()
{
  ...
  {
    auto attrs1 = std::make_unique<TorrentAttribute>();
    attrs1->infoHash = "hash1";
    auto attrs2 = std::make_unique<TorrentAttribute>();
    attrs2->infoHash = "hash2";
    btRegistry.getDownloadContext(1)->setAttribute(ContextAttributeType::CTX_ATTR_BT,
                                                   std::move(attrs1));
    btRegistry.getDownloadContext(2)->setAttribute(ContextAttributeType::CTX_ATTR_BT,
                                                   std::move(attrs2));
  }

                    ...
                    const Option* option)
{
  ...
  if (!metadataGetMode) {
    auto c = std::make_unique<PeerChokeCommand>(e->newCUID(), e);
    c->setPeerStorage(peerStorage);
    c->setBtRuntime(btRuntime);
  ...
  {
    auto c = std::make_unique<ActivePeerConnectionCommand>(
  ...
  if (metadataGetMode || !torrentAttrs->privateTorrent) {
    if (DHTRegistry::isInitialized()) {
      auto command =
          std::make_unique<DHTGetPeersCommand>(e->newCUID(), requestGroup, e);
  ...
  if (btReg->getTcpPort() == 0) {
    ...
    for (size_t i = 0; i < familiesLength; ++i) {
      auto command =
          std::make_unique<PeerListenCommand>(e->newCUID(), e, families[i]);
      bool ret;
      uint16_t port;
      if (btReg->getTcpPort()) {
        SegList<int> sgl;
  ...
  if (btStopTimeout > 0) {
    auto stopDownloadCommand = std::make_unique<BtStopDownloadCommand>(

...
AbstractSingleDiskAdaptor::fileAllocationIterator()
{
  ...
  case (DiskAdaptor::FILE_ALLOC_FALLOC):
    return std::make_unique<FallocFileAllocationIterator>(diskWriter_.get(),
                                                          size(), totalLength_);
#endif // HAVE_SOME_FALLOCATE
  case (DiskAdaptor::FILE_ALLOC_TRUNC):
    return std::make_unique<TruncFileAllocationIterator>(diskWriter_.get(),
                                                         size(), totalLength_);
  default:
    return std::make_unique<AdaptiveFileAllocationIterator>(
